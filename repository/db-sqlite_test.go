...
func TestSQLiteRepository_InsestHolding(t *testing.T) {
	h := Holdings{
		Amount:        1,
...
func TestSQLiteRepository_GetHoldingByID(t *testing.T) {
	h, err := testRepo.GetHoldingsByID(1)
	if err != nil {
		t.Fatal("get by id failed:", err)
	}
	...
	_, err = testRepo.GetHoldingsByID(2)
	if err == nil {
		t.Error("get one returned for non-existent id")
	}
}
